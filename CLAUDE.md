...
> **STATUS: RECONSTRUCTED DRAFT — pending human approval before any build.**
> The original `CLAUDE.md` was absent from the repo. This file is a best-effort
> reconstruction synthesized from: the `solana-resilience-kit` README (the §11b
> library), the live TxLINE docs (`https://txline-docs.txodds.com/llms.txt` and
> linked pages), the installed toolchain, and a standard §0→9 hackathon build
> structure. Pinned versions are real (verified via `npm view`, June 2026).
> Acceptance tests, the on-chain settlement design, and the demo scenario are
> _proposed_ and must be confirmed by the project owner. Treat every "ASSUMPTION"
> and "OPEN" marker as a review gate.
...
**ASSUMPTION:** Anchor TS client is _not_ used at runtime — the ClearLine program's
TS client is generated by **Codama** to a `@solana/kit`-native client, and every
transaction is sent through solana-resilience-kit's `TransactionSender`. This keeps
the §11b mandate ("no bare @solana/kit RPC") and avoids web3.js-v1/v2 split.
...
- **Auth:** `POST https://txline.txodds.com/auth/guest/start` → JWT (30-day);
  ...
  leagues = World Cup free tier) → API token. Header convention to verify in Phase 1:
  `Authorization: Bearer <token>` and, for stat-validation, additionally
  `X-Api-Token: <token>` — **OPEN: reconcile the two documented conventions live.**
...
## Blockers / open questions (resolve as encountered; don't stall unblocked work)

- **(a) TxLINE secrets / activation:** guest JWT can be auto-fetched; **token
  activation** needs the agent wallet to sign and _may_ need an on-chain
  purchase/tx. If the World Cup free tier (empty leagues) activates without a
  paid/manual step, the agent self-provisions. If it requires payment or manual KYC
  → **stop and ask**.
- **(b) Agent wallet:** generate a dedicated **devnet** keypair locally + airdrop —
  self-provisioned (not a blocker), kept in `.env`/gitignored.
- **(c) Cloudflare deploy:** local dev (`wrangler dev`/miniflare + local D1) needs **no
  login**, so the full build + demo-replay run locally. **Deploy** (`wrangler deploy`,
  Pages, `wrangler secret put`, remote D1 create) requires `wrangler login` — a
  **manual** step the owner performs.
- **(d) GitHub:** PRs/repo via `gh`; if `gh auth status` is unauthenticated, pushing
  is a **manual** step → work locally with branches until authed.
- **OPEN:** confirm the stat-validation auth header convention (Bearer-only vs
  Bearer + `X-Api-Token`) against the live API in Phase 1.
- **OPEN:** confirm the ClearLine settlement design (CPI vs verified-verdict record)
  after the Phase-4 spike.
